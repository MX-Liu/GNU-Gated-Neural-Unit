...
def run_single_training(args, logger):
    ...
    model = get_model(
        model_name=args.model, num_classes=metadata['num_classes'],
        transfer_learning=args.transfer_learning, freeze_layers=not args.fine_tune, classifier=args.classifier,alpha1=args.alpha1, alpha2=args.alpha2,projection=args.projection,fixed_alpha=args.fixed_alpha,dyanmic_alpha=args.dynamic_alpha,independent_alpha=args.independent_alpha,trainable_activation=args.trainable_activation
    ).to(device)
    logger.info(f"Model '{args.model}' initialized. Pre-trained: {args.transfer_learning}. Layers frozen: {not args.fine_tune}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--trainable_activation', type=str, default='none', choices=['none', 'prelu', 'psilu','snake'], help='Trainable activation function for MLP-KAN classifier.')
    ...
    elif args.trainable_activation != 'none':
        log_file = f"{folder_name}/Dataset_{args.dataset}_percentage_{args.ds_percentage}_noise_{args.noise}_model_{args.model}_classifier_{args.classifier}_activation_{args.activation}_degree_{args.degree}_grid_{args.grid}_alp1_{args.alpha1}_alp2_{args.alpha2}_epoch_{args.num_epochs}_trainable_activation_{args.trainable_activation}_seed_{args.seed}.log"
    else:
